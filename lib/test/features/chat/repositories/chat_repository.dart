  ...
  Stream<List<Message>> getGroupChatStream(String groudId) {
    return firestore
      ...
      for (var document in event.docs) {
        ...
        var messageData = Message.fromJson(document.data());
        var userData =
            await firestore.collection('users').doc(messageData.senderId).get();
        var user = UserModel.fromJson(userData.data()!);
        messages.add(
          Message(
            senderId: messageData.senderId,
            senderName: user.name,
            senderIsOnline: user.isOnline,
...
// users -> reciever user id => chats -> current user id -> set data
  ...
  void _saveMessageToMessageSubcollection({
    ...
    if (isGroupChat) {
      ...
      await firestore.collection('groups').doc(recieverUserId).update({
        'membersUid': FieldValue.arrayUnion([message.senderId]),
      });
    } else {
